<?php
	...
	echo "
	<script>
		document.title = 'Lihat Bagian | DISPLAY PRD'
	</script>
	<script>
		...
		<div class='title-admin'>
			<div style='padding:20px; height:142px; background:rgba(0,0,0,0.4)'>
				<div style='font-size:70px; margin-right:15px; margin-top:10px;' class='f-putih f-symbol'>i</div>
					<div class='f-putih f24 f-segoe' style='padding:15px; margin-top:50px;'>Lihat Bagian<br><div style='margin-top:7px;' class='f-segoe f-putih f16'><i>Data untuk melihat daftar bagian kerja...</i></div></div>
			</div>
		</div>
			...
			<div class='f14'><a href='#' class='link-biru'>DISPLAY PRD</a> <span class='f-none f-symbol1'>&#215;</span> Data</div>
				...
				<div class='overlay wrapper-load-hapus'>
					<div class='wrapper-auto bg-putih' style='width:340px; margin-top:100px'>
						...
						<form method='POST' id='form_input' action='aksi_hapus_bagian'>
		...
		<div class='wrapper-tengah'>
			<div class='wrapper-form' style='width:900px;'>
				<div class='title-form'><span class='f-symbol mr10'>i</span>DATA BAGIAN</div>
				<div class='pd15'>
					<table id='table' class='f-segoe' border='1' style='border:solid 1px rgba(0,0,0,0.3); border-collapse:collapse;'>
						<thead class='f12'>
							<tr class='title-field'>
								...
								<th class='sorting' tabindex='0' aria-controls='example' rowspan='1' colspan='1' aria-label='Office: activate to sort column ascending' style='width: 600px;'>BAGIAN</th>
								<th width='10px'>AKSI</th>
							</tr>
						</thead>
						<tbody>";
							$no = 1;
							$q = msq($kon, "SELECT * from t_bagian order by nama asc");
							...
							<script>
								...
								{	
									$.ajax({
										url:'aksi_ambil_data_hapus_bagian',
										type:'POST',
										data:'id='+data,
										dataType:'json',
										success:function(d){
							...
							<tr style='background:rgba(255,255,255,0.4); font-size:12px;'>
								...
								<td>
									<div class='wrapper-auto' style='width:115px;'>
										<div class='t-center pointer link-hapus f-symbol1' onclick='del_conf($d[id_bagian])' style='margin-right:10px; background:red; padding:5px; border-radius:3px; position:relative; color:white;'>Í<span class='f-segoe' style='margin-left:5px;'>Hapus</span></div>
										<a href='$_SESSION[level]/edit_bagian_$d[id_bagian]' class='t-center pointer link-hapus f-symbol1' style='position:relative; background:blue; padding:5px; border-radius:3px; color:white;'>?<span class='f-segoe' style='margin-left:5px;'>Edit</span></a>
									</div>
								</td>
