<?php
	...
	{
		header("Location: tambah_jpm");
	}
	else
	{
		$id = $_GET['id'];
		$q_id_jpm = msq($kon, "select * from t_jpm where id_jpm='$id'");
		$jml = mnr($q_id_jpm);
		if($jml==0)
		{
			header("location: tambah_jpm");
		...
		{
			$d_id_jpm = mfa($q_id_jpm);
		}
	}
		
	$tgl_jpm = mfa(msq($kon, "select * from t_jpm order by id_jpm desc limit 1"));
	$q_cek_jpm = msq($kon, "select count(id_jpm) as jml from t_jpm where tgl_mulai='$tgl_jpm[tgl_mulai]' and tgl_akhir='$tgl_jpm[tgl_akhir]' group by tgl_mulai, tgl_akhir");
	...
	<script>
		...
		<div class='title-admin'>
			<div style='padding:20px; height:142px; background:rgba(0,0,0,0.4)'>
				<div style='font-size:70px; margin-right:15px; margin-top:10px;' class='f-putih f-symbol'>?</div>
					<div class='f-putih f24 f-segoe' style='padding:15px; margin-top:50px;'>Tambah JPM B<br><div style='margin-top:7px;' class='f-segoe f-putih f16'><i>Form untuk menambahkan jadwal pekerjaan mingguan...</i></div></div>
			</div>
		</div>
		...
		<div class='wrapper-tengah'>
			...
			<div class='title-form'><span class='f-symbol mr10'>?</span>TAMBAH JPM SHIFT B</div>
				<div class='pd15'>
					<form method='POST' action='aksi_tambah_jpm' id='form_input'>
						<div class='form-grup f-left' style='width:48.5%; margin-right:20px;'>
							...
							<input type='text' class='form transi-3' name='tgl_mulai' readonly value='$d_id_jpm[tgl_mulai]'>
						</div>
						<div class='form-grup f-left' style='width:48.5%; margin-bottom:20px;'>
							<div class='label-form'>TGL AKHIR</div>
							<input type='text' value='$d_id_jpm[tgl_akhir]' readonly class='form transi-3' name='tgl_akhir'>
						</div> 
						<script>
									...
									$q_shift = msq($kon, "SELECT
															...
															WHERE
																id_shift NOT IN (
																	...
																	FROM
																		t_jpm_detail jd
																	INNER JOIN t_karyawan k ON jd.nik = k.nik
																	WHERE
																		id_jpm_detail = '$d_id_jpm[id_jpm]'
																	AND k.id_mesin = '$nama_pack[id_mesin]'
																)
															ORDER BY
																nama ASC");
								...
								<tbody>";
									...
									$q = msq($kon, "SELECT *, t.tugas AS nama_tugas, k.nama AS nama_karyawan, b.nama AS nama_bagian,m.nama AS nama_mesin FROM t_karyawan k
													...
													WHERE k.nik not IN (
																	...
																	WHERE
																		id_jpm_detail = '$d_id_jpm[id_jpm]'
																	AND k.id_mesin = '$nama_pack[id_mesin]' OR k.id_mesin='$nama_fill[id_mesin]'
																) and (m.id_mesin=$nama_fill[id_mesin] or m.id_mesin=$nama_pack[id_mesin]) order by m.id_bagian");
						...
						<div style='background:#eee; border-bottom: solid 1px #ccc; padding:15px;' class='tbl-mesin  mesin-$d_mes[id_mesin] pointer' onclick='show_mesin($d_mes[id_mesin])'>$d_mes[nama]<div class='f-right f-symbol1'>></div></div>
							...
							<div class='form-grup' style='margin:10px;'>
								...
								<select class='form transi-3' name='id_shift_$d_mes[id_mesin]'>
									<option>PILIH SHIFT</option>";
									$q_shift = msq($kon, "SELECT * FROM t_shift where id_shift NOT IN (
																	SELECT
																		id_shift
																	FROM
																		t_jpm_detail jd
							...
							<table class='table' width='96%'' class='f-segoe hover stripe' border='1' style='margin-left:10px !important; margin:10px !important; border:solid 1px rgba(0,0,0,0.3); border-collapse:collapse;'>
								...
								<tbody>";
									...
									$q = msq($kon, "SELECT *, t.tugas AS nama_tugas, k.nama AS nama_karyawan, b.nama AS nama_bagian,m.nama AS nama_mesin FROM t_karyawan k
													...
													INNER JOIN t_mesin m ON m.id_mesin=k.id_mesin
													INNER JOIN t_tugas t ON k.tugas=t.id_tugas
													WHERE k.id_mesin=$d_mes[id_mesin] and k.nik not IN (
														SELECT
															k.nik
														FROM
															t_jpm_detail jd
